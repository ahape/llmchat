...
def load_models_csv(csv_path="models.csv"):
...
def find_model_info(model_name, provider=None):
...
def get_cheapest_provider(model_name):
    ...
    matches = find_model_info(model_name)
    if not matches:
        return None
...
def get_fastest_provider(model_name):
...
def list_models(search=None):
    ...
    for model_info in models:
        ...
        if model_name not in seen_models:
            ...
            cheapest = get_cheapest_provider(model_name)
            if cheapest:
                table.add_row(
                    cheapest['Model'],
                    cheapest['Provider'],
...
def set_default_model(model, provider=None):
    ...
    # Display model info
    if provider:
        matches = find_model_info(model, provider)
        model_info = matches[0] if matches else None
    else:
        model_info = get_cheapest_provider(model)
...
def format_model_with_provider(model, provider):
...
def prompt(question, model=None, stream=True, system_prompt=None, provider=None):
    ...
    if provider:
        model_with_provider = format_model_with_provider(model, provider)
    else:
    ...
    try:
        if stream:
            answer = stream_answer(question, model_with_provider, system_prompt)
        else:
            answer = get_answer(question, model_with_provider, system_prompt)
